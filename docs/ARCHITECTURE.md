...
@export var hitbox_from_sprite: bool = false # trace collision polygons from the idle frame's opaque pixels (replaces the rect shapes at load)
...
- Visuals: `drone_boss_*` sheets (64×64 frames, generated by `art/scripts/drone_boss_*.py` via the pixel-art-32 pipeline; frame transforms in `art/scripts/drone_boss_sheets.py`). `hitbox_from_sprite = true` — collision matches the sprite's opaque pixels. HP bar sits at y = -38 to clear the 64 px sprite.
